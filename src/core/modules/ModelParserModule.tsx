...
type THREEObjectType = THREE.Bone | THREE.LineSegments | THREE.LineLoop | THREE.Points | THREE.Group | THREE.Mesh | THREE.InstancedMesh | THREE.Scene | THREE.Sprite | THREE.LOD | THREE.SkinnedMesh | THREE.Camera | THREE.PerspectiveCamera | THREE.OrthographicCamera | THREE.CubeCamera | THREE.ArrayCamera | THREE.Object3D;

type THREECameraType = THREE.PerspectiveCamera | THREE.OrthographicCamera;

export type GLTFResult = GLTF & {
	nodes: Record<string, THREEObjectType>,
	materials: Record<string, THREE.Material>
}

// TODO: Fix typings
export function parse(gltfResult: GLTFResult, options = defaultOptions) {
	const animations = gltfResult.animations;
	// TODO: Add support for animations
	const hasAnimations = animations.length > 0;

	// Collect all objects
	const objects: THREEObjectType[] = [];
	gltfResult.scene.traverse((child) => objects.push(child))
	...
	const uniqueName = (attempt: string, index = 0): string => {
	...
	gltfResult.scene.traverse((child) => {
		if ((child as THREE.Mesh).isMesh) {
			...
			if (childMesh.material) {
				if (!duplicates.materials[(childMesh.material as THREE.Material).name]) {
					duplicates.materials[(childMesh.material as THREE.Material).name] = 1
				} else {
					duplicates.materials[(childMesh.material as THREE.Material).name]++
				}
			}
			if (childMesh.geometry) {
				if (!duplicates.geometries[childMesh.geometry.uuid]) {
					let name = (child.name || "Part").replace(/[^a-zA-Z]/g, "")
					name = name.charAt(0).toUpperCase() + name.slice(1)
					duplicates.geometries[childMesh.geometry.uuid] = {
						count: 1,
						name: uniqueName(name),
						node: "nodes" + sanitizeName(child.name),
					}
				} else {
					duplicates.geometries[childMesh.geometry.uuid].count++
				}
			}
		}
	});
	...
	const getType = (obj: THREEObjectType) => {
	...
	const handleProps = (obj: THREEObjectType) => {
		...
		if (
			...
			(options.keepnames ||
				...
				(animated &&
					gltfResult.animations.find((clip) => clip.name.includes(obj.name))
				)
			)
		) {
			objectProps["name"] = `${obj.name}`;
		}

		const isCamera = type === "PerspectiveCamera" || type === "OrthographicCamera"
		// Handle cameras
		if (isCamera) {
			const camera = (obj as THREECameraType)
			objectProps["makeDefault"]=false;
			if (camera.zoom !== 1) objectProps["zoom"] = rNbr(camera.zoom);
			if (camera.far !== 2000) objectProps["far"] = rNbr(camera.far);
			if (camera.near !== 0.1) objectProps["near"] = rNbr(camera.near);
		}
		if (type === "PerspectiveCamera") {
			const perspectiveCamera = (obj as THREE.PerspectiveCamera)
			if (perspectiveCamera.fov !== 50) objectProps["fov"] = rNbr(perspectiveCamera.fov);
		}

		if (!instanced) {
			// Shadows
			if (type === "mesh" && options.shadows) {
				objectProps["castShadow"] = true;
				objectProps["receiveShadow"] = true;
			}

			// Write out geometry first
			if ((obj as THREE.Mesh).geometry) {
				objectProps["geometry"] = (node as THREE.Mesh).geometry;
			}

			// Write out materials
			if ((obj as THREE.Mesh).material) {
				const objMesh = (obj as THREE.Mesh);
				const objMeshMaterial = objMesh.material as THREE.Material
				if (objMeshMaterial.name) {
					objectProps["material"] = gltfResult.materials[objMeshMaterial.name];
				} else {
					objectProps["material"] = (node as THREE.Mesh).material;
				}
			}

			// TODO: Fix typings
			if ((obj as any).skeleton) objectProps["skeleton"] = (node as any).skeleton;
			if (obj.visible === false) objectProps["visible"] = false;
			if (obj.castShadow === true) objectProps["castShadow"] = true;
			if (obj.receiveShadow === true) objectProps["receiveShadow"] = true;
			if ((obj as THREE.Mesh).morphTargetDictionary) objectProps["morphTargetDictionary"] = (node as THREE.Mesh).morphTargetDictionary;
			if ((obj as THREE.Mesh).morphTargetInfluences) objectProps["morphTargetInfluences"] = (node as THREE.Mesh).morphTargetInfluences;
			// Lights have intensity
			if ((obj as THREE.Light).intensity && rNbr((obj as THREE.Light).intensity)) objectProps["intensity"] = rNbr((obj as THREE.Light).intensity);
			//if (obj.power && obj.power !== 4 * Math.PI) result += `power={${rNbr(obj.power)}} `
			if ((obj as THREE.SpotLight).angle && (obj as THREE.SpotLight).angle !== Math.PI / 3) objectProps["angle"] = rDeg(`${(obj as THREE.SpotLight).angle}`);
			if ((obj as THREE.SpotLight).penumbra && rNbr((obj as THREE.SpotLight).penumbra) !== 0) objectProps["penumbra"] = rNbr((obj as THREE.SpotLight).penumbra);
			if (
				((obj as THREE.SpotLight).decay && rNbr((obj as THREE.SpotLight).decay) !== 1) ||
				((obj as THREE.PointLight).decay && rNbr((obj as THREE.PointLight).decay) !== 1)
			) {
				objectProps["decay"] = rNbr((obj as any).decay);
			}
			if (
				(obj as THREE.SpotLight).distance && rNbr((obj as THREE.SpotLight).distance) !== 0 ||
				(obj as THREE.PointLight).distance && rNbr((obj as THREE.PointLight).distance) !== 0
			) {
				objectProps["distance"] = rNbr((obj as any).distance);
			}
			if (
				obj.up && 
				obj.up.isVector3 && 
				...
				objectProps["up"] = [rNbr(obj.up.x), rNbr(obj.up.y), rNbr(obj.up.z)];
		}

		if ((obj as THREE.Light).color && (obj as THREE.Light).color.getHexString() !== "ffffff") {
			objectProps["color"] = `#${(obj as THREE.Light).color.getHexString()}`
		}
		if (obj.position && obj.position.isVector3 && rNbr(obj.position.length())) {
			objectProps["position"] = [rNbr(obj.position.x), rNbr(obj.position.y), rNbr(obj.position.z)];
		}
		if (
			obj.rotation && 
		...
		if (
			...
			if (rX === rY && rX === rZ) {
				objectProps["scale"] = rX;
			} else {
				objectProps["scale"] = [rX, rY, rZ];
			}
		}
		if (options.meta && obj.userData && Object.keys(obj.userData).length)
			objectProps["userData"] = JSON.stringify(obj.userData);

		return objectProps
	}

	const getObjectInfo = (obj: Exclude<THREEObjectType, "THREE.Skeleton">) => {
		const type = getType(obj)
		const node = gltfResult.nodes[obj.name];
		const instanced =
			(options.instance || options.instanceall) &&
			(obj as THREE.Mesh).geometry &&
			duplicates.geometries[(obj as THREE.Mesh).geometry.uuid] &&
			duplicates.geometries[(obj as THREE.Mesh).geometry.uuid].count > (options.instanceall ? 0 : 1)
		const animated = gltfResult.animations && gltfResult.animations.length > 0
		return { type, node, instanced, animated }
	}

	const print = (
		objects: THREEObjectType[], 
		gltf: GLTF, 
		obj: THREEObjectType, 
		...
		if (options.setLog)
			setTimeout(
				options.setLog ? () => (options.setLog as any)((state: any) => [...state, obj.name]) : () => {},
		...
		if (obj.children) {
			obj.children.forEach((child) => {
				children.push(print(objects, gltf, child))
			})
		}

		// TODO: Add support for instanced objects
		// if (instanced) {
		// 	result = `<instances.${duplicates.geometries[(obj as THREE.Mesh).geometry.uuid].name} `
		// } else {
		// Form the object in JSX syntax
		const componentName = type;
		// }

		let componentProps = handleProps(obj);
		...
		if (!options.keepgroups && !animated && (type === "group" || type === "scene")) {
			 ...
			 */
			if (isEmpty(componentProps) || obj.children.length === 0) {
				console.log("group removed (empty)");
				(obj as any).__removed = true
				return children
			}

			// TODO: Add aggressive removal strategies
			// if (options.aggressive) {
			// 	const equalOrNegated = (a: THREE.Euler, b: THREE.Euler) => {
			...
			// 			(obj as any).__removed = (obj as any).__removed = true
			// 			children = ""
			...
			// 	})
			// 	if (!empty.length) {
			// 		empty.forEach((o) => (o.__removed = true))
			// 		return ""
			// 	}
			// }
		}
		...
		// Inject properties
		componentProps = Object.assign(componentProps, injectedProps);
	...
	// TODO: Add support for animations
	const printAnimations = (animations: THREE.AnimationClip[]) => {
	...
	const debug = (obj: THREEObjectType, line: number) => {
		console.log(
			...
			"mat:",
			(obj as THREE.Mesh).material ? `${((obj as THREE.Mesh).material as THREE.Material).name}-${((obj as THREE.Mesh).material as THREE.Material).uuid.substring(0, 8)}` : ""
		)
		obj.children.forEach((o) => debug(o, line + 1))
	}

	if (options.debug) debug(gltfResult.scene, 0)

	// const instancedModel = hasInstances
	// ? `
	...
	// `
	// : "";
	// const modelExportString = `${hasInstances ? "" : "export const"} Model = (
	// { 
	// ${hasInstances ? "instances, " : ""}...props 
	// }) => {`;
	// const groupRefString = `const group = ${ options.types ?"useRef<THREE.Group>()": "useRef()"}`
	// const gltfString = `
	// 	const { 
	// 		nodes, 
	// 		materials, 
	// 		${hasAnimations ? "animations" : ""} 
	// 	} = useGLTF('${url}'``${options.draco ? `, ${JSON.stringify(options.draco)}` : ""}) ${options.types ? " as GLTFResult" : ""}${printAnimations(animations)}
	// `
